...
## verify-selected：按需核验

`verify-selected DIRECTORY MANIFEST PUBLIC SELECTION` 只信任命令行指定的 PEM Ed25519 公钥，兼容清单版本 1 与版本 2，但**只核验 SELECTION 点名的文件**，不扫描交付目录中的其他任何内容。

SELECTION 是一个位于交付树**之外**的 UTF-8 JSON 文件，内容为**非空、字符串唯一的数组**；每项必须与某条 `files.path` **精确相等**。下列选择项一律拒绝（原因写标准错误、返回 2，且不读取任何交付文件）：绝对路径（以 `/` 开头）、空段、`.` 或 `..` 段、含反斜杠的路径，以及清单中不存在的路径。MANIFEST、PUBLIC、SELECTION 都不得位于交付目录内。

核验严格分两阶段：

1. **先建立信任**：校验清单结构，再核对版本 2 清单的 `key_id` 与该公钥一致，最后验证签名。任一不符：输出 `valid:false`、`reason:"untrusted_manifest"`、`checked:0`，`modified` 与 `missing` 均为空数组，返回 1，且**不读取任何交付文件**。
2. **可信后才读文件**：仅按**清单顺序**读取所选文件（不按 SELECTION 顺序，也不扫描其他内容）。逐级检查路径上的每个分量，拒绝符号链接和特殊文件；读取使用 `O_NOFOLLOW`，并在读取前后同时核对路径与打开句柄的状态——类型、`(设备号, inode)` 身份、大小和纳秒修改时间。读取期间文件被替换或发生任何变化：原因写标准错误、返回 2。

- 全部所选文件匹配：输出 `valid:true` 和已核验文件数 `checked`，返回 0。
- 所选文件被篡改或缺失：输出 `valid:false`、`reason:"file_mismatch"`、`checked`（成功读取并比对过的文件数）以及按路径排序的 `modified`、`missing` 列表，返回 1。不报告 `unexpected`——未选择的文件从不检查。
- 参数、SELECTION 或清单格式错误、I/O 错误、路径冲突或读取期间树发生变化：原因写标准错误，返回 2。
...
`verify-batch BATCH` 从一个 BATCH 文件依次执行多项校验。BATCH 是一个 UTF-8 JSON **数组**，每项恰好包含三个字段：`id`（唯一、非空字符串）、`command`（`verify`、`verify-selected`、`verify-trusted` 或 `verify-policy`）和 `args`（字符串数组，长度与命令匹配：`verify`/`verify-trusted` 为 3，`verify-selected`/`verify-policy` 为 4），含义和顺序与对应单命令的位置参数完全一致。`args` 中的相对路径基于 **BATCH 文件所在目录**解析；BATCH 文件本身必须位于各交付树之外，各项仍沿用原命令的全部限制（配套文件在交付树外、`O_NOFOLLOW`、前后快照）。
...
### audit-chain BATCH PREVIOUS REPORT：哈希链审计报告

`audit-chain BATCH PREVIOUS REPORT` 执行与 `audit-batch` 完全相同的批量校验（BATCH 格式、相对路径基准、执行顺序、目录限制、空批次合法性和返回码都一致），并把本次运行追加为一份**哈希链**审计报告 REPORT。PREVIOUS 为 `-` 时创建链起点；否则 PREVIOUS 必须是一份合法的链报告，新报告接在其后。BATCH 结构错误、PREVIOUS 缺失或非法、任何 I/O 错误：原因写标准错误、返回 2、**不创建报告**。

REPORT 是 UTF-8 JSON 对象，恰好包含八个字段：`kind`（`"release-seal-audit-chain"`）、`version`（`2`）、`sequence`、`previous_sha256`、`batch_sha256`、`valid`、`summary` 和 `results`。起点的 `sequence` 为 1、`previous_sha256` 为 `null`；后继报告的 `sequence` 为前序序号加一、`previous_sha256` 为前序报告文件**原始字节**的 SHA-256 小写十六进制。其余字段沿用版本 1 审计报告语义；报告不记录参数、密钥材料、签名或 traceback。REPORT 与 PREVIOUS 都必须位于所有交付树之外，REPORT 绝不覆盖已有文件，发布语义（同步临时文件 + 不覆盖硬链接 + 目录同步）与 `audit-batch` 完全一致。

### audit-chain-verify EXPECTED_HEAD REPORT...：校验审计链

`audit-chain-verify EXPECTED_HEAD REPORT...` 按链序接收一份或多份链报告，依次核对：首份必须是链起点（`sequence` 为 1、`previous_sha256` 为 `null`）、序号连续递增、每份的 `previous_sha256` 与前一份文件原始字节的 SHA-256 一致，且末份报告原始字节的 SHA-256 等于 EXPECTED_HEAD（64 位小写十六进制）。
...
所有命令沿用相同的目录限制：输入必须是目录，目录树不支持符号链接和特殊文件（读取文件时使用 `O_NOFOLLOW`，防止检查后被换成符号链接）。私钥、公钥、清单、增量清单、选择文件（SELECTION）、信任库、策略、BATCH 文件和审计报告不得位于交付目录内——既包括命令行参数，也包括盘点时在交付树中发现的同类文件。树内识别**只看内容、不看文件名或扩展名**。
