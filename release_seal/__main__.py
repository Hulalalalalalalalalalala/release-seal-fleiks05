...
def main() -> int:
    ...
    multisig = commands.add_parser(
        "sign-multi",
        help="sign with several Ed25519 private keys (version 3 manifest)",
    )
    multisig.add_argument("directory", help="directory to read")
    multisig.add_argument("manifest", help="manifest to create; must not exist yet")
    multisig.add_argument(
    ...
    policy_check = commands.add_parser(
        "verify-policy",
        help="verify a multisig manifest against a threshold policy",
    )
    policy_check.add_argument("directory", help="directory to check")
    policy_check.add_argument("manifest", help="version 3 multisig manifest")
    ...
    try:
        ...
        if args.command == "inventory":
            ...
            from .seal import sign_multi

            result = sign_multi(args.directory, args.manifest, args.private)
            code = 0
